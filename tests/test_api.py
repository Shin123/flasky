...
class APITestCase(unittest.TestCase):
    ...
    def test_bad_auth(self):
        ...
        u = User(email='jonh@example.com', password='cat', confirmed=True, role=r)
        db.session.add(u)
        db.session.commit()
        ...
        response = self.client.get(
            '/api/v1/posts/',
            headers=self.get_api_headers('jonh@example.com', 'dog')
        )
        self.assertEqual(response.status_code, 401)
    ...
    def test_token_auth(self):
        ...
        response = self.client.post(
            '/api/v1/tokens/',
            headers=self.get_api_headers('jonh@example.com', 'cat')
        )
    ...
    def test_users(self):
        ...
        response = self.client.get(
            ...
            headers=self.get_api_headers(u2.email, 'dog')
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
    ...
    def test_comments(self):
        ...
        response = self.client.post(
            ...
            headers=self.get_api_headers(u2.email, 'dog'),
            data=json.dumps({'body': 'Good! Comment in test'})
        )
        self.assertEqual(response.status_code, 201)
        json_response = json.loads(response.get_data(as_text=True))
        url = response.headers.get('Location')
        self.assertEqual(json_response['body'], 'Good! Comment in test')
        
        # get the new comment
        response = self.client.get(
            url,
            headers=self.get_api_headers(u1.email, 'cat')
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['url'], url)
