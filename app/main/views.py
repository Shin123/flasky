from flask import make_response, render_template, session, redirect, url_for, current_app, flash, abort, request
import werkzeug
...
def index():
    ...
    if show_followed:
        ...
        query = Post.query
    pagination = query.order_by(Post.timestamp.desc()).paginate(page=page,per_page=current_app.config['FLASK_POSTS_PER_PAGE'],
         error_out=False)
...
def show_all():
    ...
    resp.set_cookie('show_followed', '', max_age=30*24*60*60) # 30day
    return resp
...
def show_followed():
    resp = make_response(redirect(url_for('.index')))
    resp.set_cookie('show_followed', '1', max_age=30*24*60*60) # 30day
    return resp
